...
MemoryCache.ResourceRemovedListener {
    ...
    private EngineResource<?> loadFromActiveResources(Key key, boolean bl) {
        ...
        if ((object = (EngineResource)((Reference)object).get()) != null) {
    ...
    private static void logWithTimeAndKey(String string2, long l, Key key) {
        Log.v((String)"Engine", (String)(string2 + " in " + LogTime.getElapsedMillis(l) + "ms, key: " + key));
    }

    public <T, Z, R> LoadStatus load(Key key, int n, int n2, DataFetcher<T> object, DataLoadProvider<T, Z> dataLoadProvider, Transformation<Z> transformation, ResourceTranscoder<Z, R> resourceTranscoder, Priority priority, boolean bl, DiskCacheStrategy diskCacheStrategy, ResourceCallback resourceCallback) {
        Util.assertMainThread();
        long l = LogTime.getLogTime();
        Object object2 = object.getId();
        key = this.keyFactory.buildKey((String)object2, key, n, n2, dataLoadProvider.getCacheDecoder(), dataLoadProvider.getSourceDecoder(), transformation, dataLoadProvider.getEncoder(), resourceTranscoder, dataLoadProvider.getSourceEncoder());
        object2 = this.loadFromCache(key, bl);
        if (object2 != null) {
            resourceCallback.onResourceReady((Resource<?>)object2);
            if (Log.isLoggable((String)"Engine", (int)2)) {
                Engine.logWithTimeAndKey("Loaded resource from cache", l, key);
            }
            return null;
        }
        ...
        if (object2 != null) {
            resourceCallback.onResourceReady((Resource<?>)object2);
            if (Log.isLoggable((String)"Engine", (int)2)) {
                Engine.logWithTimeAndKey("Loaded resource from active resources", l, key);
            }
            return null;
        }
        object2 = this.jobs.get(key);
        if (object2 != null) {
            ((EngineJob)object2).addCallback(resourceCallback);
            if (Log.isLoggable((String)"Engine", (int)2)) {
                Engine.logWithTimeAndKey("Added to existing load", l, key);
            }
            return new LoadStatus(resourceCallback, (EngineJob)object2);
        }
        object2 = this.engineJobFactory.build(key, bl);
        object = new EngineRunnable((EngineRunnable.EngineRunnableManager)object2, new DecodeJob<T, Z, R>((EngineKey)key, n, n2, (DataFetcher<T>)object, dataLoadProvider, transformation, resourceTranscoder, this.diskCacheProvider, diskCacheStrategy, priority), priority);
        this.jobs.put(key, (EngineJob)object2);
        ((EngineJob)object2).addCallback(resourceCallback);
        ((EngineJob)object2).start((EngineRunnable)object);
        if (Log.isLoggable((String)"Engine", (int)2)) {
            Engine.logWithTimeAndKey("Started new load", l, key);
        }
        return new LoadStatus(resourceCallback, (EngineJob)object2);
    }
