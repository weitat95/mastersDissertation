...
implements ResourceEncoder<GifDrawable> {
    ...
    private GifDecoder decodeHeaders(byte[] arrby) {
        ...
        ((GifHeaderParser)object).setData(arrby);
    ...
    private boolean writeDataDirect(byte[] arrby, OutputStream outputStream) {
        try {
            ...
            if (Log.isLoggable((String)"GifEncoder", (int)3)) {
                Log.d((String)"GifEncoder", (String)"Failed to write data to output stream in GifResourceEncoder", (Throwable)iOException);
            }
            return false;
        }
    }
    ...
    public boolean encode(Resource<GifDrawable> object, OutputStream resource) {
        ...
        Transformation<Bitmap> transformation = ((GifDrawable)(object = (GifDrawable)object.get())).getFrameTransformation();
        if (transformation instanceof UnitTransformation) {
            return this.writeDataDirect(((GifDrawable)object).getData(), (OutputStream)((Object)resource));
        }
        GifDecoder gifDecoder = this.decodeHeaders(((GifDrawable)object).getData());
        ...
        Log.v((String)"GifEncoder", (String)("Encoded gif with " + gifDecoder.getFrameCount() + " frames and " + ((GifDrawable)object).getData().length + " bytes in " + LogTime.getElapsedMillis(l) + " ms"));
        return bl;
    }
