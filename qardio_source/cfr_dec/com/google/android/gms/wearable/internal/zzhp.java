...
public class zzhp {
     ...
     */
    private static boolean zza(PackageInfo arrby, byte[] ... arrby2) {
        if (arrby.signatures != null) {
            if (arrby.signatures.length != 1) {
                Log.w((String)"WearSignatureVerifier", (String)"Package has more than one signature.");
                return false;
            }
            arrby = arrby.signatures[0].toByteArray();
            for (int i = 0; i < arrby2.length; ++i) {
                if (!Arrays.equals(arrby2[i], arrby)) continue;
                return true;
            }
        }
        return false;
    ...
    private static boolean zzc(PackageInfo packageInfo, boolean bl) {
        ...
        if (bl) {
            return zzhp.zza(packageInfo, zzlme, zzlmf);
        }
        return zzhp.zza(packageInfo, new byte[][]{zzlme});
    }
     ...
     */
    public static zzhp zzep(Context context) {
        zzbq.checkNotNull(context);
        synchronized (zzhp.class) {
            if (zzlmd == null) {
                zzlmd = new zzhp(context);
            }
            return zzlmd;
        }
    }

    private static byte[] zzfx(String arrby) {
        try {
     ...
     * Enabled aggressive block sorting
     */
    public final boolean zznz(String string2) {
        boolean bl;
        block5: {
            boolean bl2;
            block4: {
                bl2 = false;
                if ((string2 = this.zzoa(string2)) == null) break block4;
                if (zzp.zzch(this.mContext)) {
                    return zzhp.zzc((PackageInfo)string2, true);
                }
                bl2 = bl = zzhp.zzc((PackageInfo)string2, false);
                if (bl) break block4;
                bl2 = bl;
                if (zzhp.zzc((PackageInfo)string2, true)) break block5;
            }
            return bl2;
        }
        Log.w((String)"WearSignatureVerifier", (String)"Test-keys aren't accepted on this build.");
        return bl;
    }
}
